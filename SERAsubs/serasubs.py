...
def load_bundled_models(model_name):
    ...
    # Check if models are detected by script (Avoid downloading the models a bajilion times)
    if "base.pt" in os.listdir(model_dir) and  "large-v3.pt" in os.listdir(model_dir) and  "small.pt" in os.listdir(model_dir):
        return whisper.load_model(model_name, download_root=model_dir)
    else:
        print("Models are not detected! Ensure you are in the same directory as this script, then run this script again.")

# --- Not needed, using system python, tkinter and ffmpeg --- 
...
class Main:
    def __init__(self, root):
        ...
        self.root = root
# --- Causes issues with tkinter for some reason ---       self.root.iconbitmap(resource_path("logo_256.ico")) 
